...
import { getSchema } from "@plugins/controllers/CrudFunctions";
...
export class CrudController {
    ...
    static async list(req: Request, res: Response) {
        ...
        const schema = await getSchema(crud);
        ...
        let totalPages: any = (await queryTotal.count().first().catch((e) => {console.error(e)}))?.count ?? 1;
        const registros: any = await query.catch((e) => console.error(e));
    ...
    static async insert (req: Request, res: Response) {
        ...
        if(![null,undefined].includes(data)){
            await query.insert({...data}).then((response: Record<string,any>) => {
